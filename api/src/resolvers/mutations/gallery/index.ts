...
export const deleteGallery: MutationDeleteGalleryResolver<{}, {}, { db: Knex }> = async (
  ...
  try {
    ...
    const [ photoQuantity ] = await db('photo').where({ galleryID }).count();
    const photoCount: number = parseInt(photoQuantity.count, 10);

    // if there are photos delete all from S3
    if (photoCount) {
      // get photoIDs
      const photos: Array<{ filename: string }> = await db.select('filename').from('photo').where({ galleryID });
      ...
      if (Deleted && !!Deleted.length) {
        ...
        await Deleted.map(({ Key: filename }) => db('photo').where({ filename }).del().returning('photoID'));
      }
    }
